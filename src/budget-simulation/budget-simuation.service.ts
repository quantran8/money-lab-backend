...
import { BillReserveOptionCode, CommitmentLayer, JarCode, SpendModeCode } from './budget-simulation.enum';
...
export class BudgetSimulationService {
  ...
  async getSetupOptions() {
    return this.wrapAsync('getSetupOptions', async () => {
      const moduleId = 3;
      const layerList = [CommitmentLayer.bills, CommitmentLayer.locked, CommitmentLayer.foodReserve];
      const [
        jobs,
        commitmentTemplates,
        ...
        moduleId !== undefined && layerList.length > 0
          ...
          : this.prisma.commitmentTemplate
  ...
  async getActiveBudgetRun(userId: string) {
    return this.wrapAsync('getActiveBudgetRun', async () => {
      ...
      const housingUtilityModifiers =
        await this.prisma.housingUtilityModifier.findMany({
          where: { housingCommitmentId: housingId },
        });
  ...
  async startMonth(
    ...
    return this.wrapAsync('startMonth', async () => {
      ...
      if (prevMonth) {
        ...
        const prevAllocs = await this.prisma.budgetMonthAllocation.findMany({
          where: {
            budgetMonthId: prevMonth.id,
            jarCode: { in: coreJars },
          },
        });
        const prevSpendByJar =
          await this.prisma.budgetMonthSpendByJar.findMany({
            where: {
              budgetMonthId: prevMonth.id,
              jarCode: { in: coreJars },
            },
        ...
        const allocByJar = Object.fromEntries(
          prevAllocs.map((a) => [a.jarCode, a.amount]),
        );
        const spendByJar = Object.fromEntries(
          prevSpendByJar.map((s) => [
            ...
            {
              spent: s.spentAmount,
              overflowIn: s.overflowInAmount,
              overflowOut: s.overflowOutAmount,
            },
          ]),
        );
      ...
      const billTemplates = await this.prisma.commitmentTemplate.findMany({
        where: { moduleId: 3, layer: CommitmentLayer.bills },
      });
      ...
      await this.upsertMonthAllocations(month.id, finalAllocations);
    ...
    return this.wrapAsync('resolveWeek', async () => {
      ...
      if (month.currentWeek >= 1) {
        const pending = await this.prisma.budgetMonthEvent.findFirst({
          where: {
            budgetMonthId: monthIdBig,
            week: month.currentWeek,
            chosenOptionId: null,
          },
        });
        if (pending)
      ...
      if (nextWeek === 4 && !eventPending) {
        ...
        const futureYouAllocation =
          await this.prisma.budgetMonthAllocation.findFirst({
            where: { budgetMonthId: monthIdBig, jarCode: JarCode.futureYou },
          });
        const futureYouSpending =
          await this.prisma.budgetMonthSpendByJar.findFirst({
            where: { budgetMonthId: monthIdBig, jarCode: JarCode.futureYou },
          });

        if (futureYouAllocation) {
          futureRemainInMonth =
            futureYouAllocation.amount - (futureYouSpending?.spentAmount ?? 0);
        }
  ...
  async applyEventChoice(
    ...
    return this.wrapAsync('applyEventChoice', async () => {
      ...
      const option = await this.prisma.lifeEventOption.findUnique({
      ...
      const paymentBreakdown =
        paymentRecord.length > 0
          ? (paymentRecord.map(({ jar, amount }) => ({
              jarCode: jar,
              amount,
            })) as { jarCode: string; amount: number }[])
          : {};
  ...
  private async finalizeBillsForMonth(
    ...
    if (
      month.billsActual !== 0 &&
      month.billsActual !== null &&
      month.billsActual !== month.billsEstimated
    ) {
  ...
  async prepareNextMonth(userId: string, runId: number) {
    return this.wrapAsync('prepareNextMonth', async () => {
      ...
      const housingId = billingCommitments[0]?.commitmentTemplateId;
      let housingUtilityModifiers: any[] = [];
      if (housingId) {
        housingUtilityModifiers =
          await this.prisma.housingUtilityModifier.findMany({
            where: { housingCommitmentId: housingId },
      ...
      const billTemplates = await this.prisma.commitmentTemplate.findMany({
        where: { moduleId: run.moduleId, layer: CommitmentLayer.bills },
      });
      ...
      // 5. Carry Over (Jars and Free Cash)
      const jarOrder = [
        JarCode.fun,
        JarCode.learning,
        JarCode.give,
      ...
      for (const jarCode of jarOrder) {
        ...
        const balance =
          allocation -
          (spend?.spentAmount ?? 0) +
          (spend?.overflowInAmount ?? 0) -
          (spend?.overflowOutAmount ?? 0);
        jarsCarryOver[jarCode] = Math.max(0, balance);
      }

      // Calculate current Free Cash balance properly
      ...
      // 6. Flexible Income
      // Formula in prompt: flexible_income = final_income - locked_commitments_total - estimated_bills - reserve_refill
