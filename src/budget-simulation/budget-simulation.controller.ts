...
export class BudgetController {
  ...
  updateRunCommitments(
    @Request() req: { user?: { id: string } },
    @Param('runId') runId: string,
    @Body() body: UpdateRunCommitmentsDto,
  ) {
    return this.budgetService.updateRunCommitments(
      getUserId(req),
      parseInt(runId),
      body.commitmentAmounts as Record<number, number>,
      body.optionals,
